...
export default function RootLinkWithChildren(props: RootLinkWithChildren) {
  ...
  return (
    <li>
      <button
        type="button"
        className="flex px-3 py-2 items-center hover:bg-gray-500 hover:text-white w-full"
      ...
      <ul
        className={
          "ml-16 border-l-gray-300 border-l-2 mt-2 border-l-solid " + hidden
        }
      >
        {menuItem.children &&
          menuItem.children.map((subItem) => {
            const isSelectedClassNames = url.startsWith(subItem.to)
              ? " bg-gray-500 text-white"
              : "";

            return (
              <Link href={subItem.to} onClick={() => setOpenNav(false)} key={subItem.key}>
                <li
                  className={
                    "pl-2 py-2 hover:bg-gray-500 hover:text-white" +
